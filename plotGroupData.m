...
for subj = subj_array_force
    ...
    for i = [2 4]
        ...
        %% ML/X dir
        % Power IP
        meanPmagX(n,i/2) = nanmean(GroupStats.meanAbsPowerIntPtX(rows));
        SDPmagX(n,i/2) = nanmean(GroupStats.SDAbsPowerIntPtX(rows));
...
%         meanNegWorkIntPtX(n,i/2) = nanmean(GroupStats.meanNegWorkIntPtX(rows));
        ...
        meanFx(n,i/2) = nanmean(GroupStats.meanFx(rows));
...
%% Compare positive vs. negative force and power mean magnitude for Assist Beam
...
xlim([0.5 2.5]);
set(gca,'xticklabel',xlab,'xtick',1:2); box off; set(gca,'tickdir','out');
title('Power IP magnitude'),ylabel('(W)');
