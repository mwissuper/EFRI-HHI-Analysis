...
if beam == 1
    ...
    % trial. issues with additional steps after trial ended and heel went up so
    % stop is too late. Just fix manually.
    ...
    % work very consistently. Just go through and pick by eye.
    % if min([stop1 stop2]) >= length(left_heel)-2 % can't find a peak if not >= 3 samples left, happens when not find beamHt correctly (need to fix manually in mainWorkPowerAnalysisMW code)
    %     stop_idx = min([stop1 stop2]);
    % else
    %     if stop1 < stop2 % look for min on left side
    %         [troughs,iTroughs] = findpeaks(-left_heel(stop1:end),'MinPeakProminence',range(left_heel)/50);
    %     else
    %         [troughs,iTroughs] = findpeaks(-right_heel(stop2:end),'MinPeakProminence',range(left_heel)/50);
    %     end
    %     if isempty(iTroughs) % too flat, no obvious troughs for HS event
    %         stop_idx = min([stop1 stop2]);
    %     else
