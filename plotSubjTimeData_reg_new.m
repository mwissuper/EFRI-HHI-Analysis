...
for subj = 8%subj_array_force
    ...
    for i = 1:length(TrialData)
        ...
        if strcmp(TrialData(i).Info.Condition,'Assist Beam') %|| strcmp(TrialData(i).Info.Condition,'Assist Ground') 
            ...
            % Extra plot for display figure
            % Plot POB Clavicle AP disp. (relative to AP pos at start) vs. time 
%             plot(TrialData(i).Results.time(3:end),TrialData(i).Results.clavY(3:end)-nanmean(TrialData(i).Results.clavY(3)),'k');
%             % Labels and formatting
%             ylabel('Clav AP'); box off; set(gca,'tickdir','out'); xlabel('Time (s)');
            ...
            subplot(numrows,numcols,plotind)
            clear temp;
            temp.c = TrialData(i).Results.cz_clav;
            temp.c(isnan(temp.c)) = 0;
            ...
            titlename = sprintf('R^2=%.2f m:%.2f b:%.2f k:%.2f',TrialData(i).Results.rsqz_clav,TrialData(i).Results.cz_clav(1),TrialData(i).Results.cz_clav(2),TrialData(i).Results.cz_clav(3));
            ...
            pdfname = sprintf('HHI%i_modelF_power_AssistBeam',subj);
            set(gcf, 'Color', 'w'); 
